...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const pairs = await prisma.pair.findMany({
    where: { id: { in: pairIds }, teamId: user.teamId, isActive: true },
  ...
  try {
    await prisma.$transaction(
      async (tx) => {
        ...
        if (!matchup || ![matchup.homeTeamId, matchup.awayTeamId].includes(user.teamId)) {
        ...
        const before = await tx.lineup.findUnique({
          where: { matchupId_teamId: { matchupId: id, teamId: user.teamId } },
          include: { slots: true },
        });
        ...
        const lineups = await tx.lineup.findMany({ where: { matchupId: id }, include: { slots: true } });
        if (lineups.length === 2) {
