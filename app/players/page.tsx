...
export default async function Players({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-8"><div className="label">Confirmed tournament participants</div><div className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-4xl font-black uppercase">Players</h1><p className="mt-2 max-w-3xl text-gray-500">Only confirmed players in public divisions are shown here. Organizers can keep tentative Executive candidates private in the admin player pool until attendance is final.</p></div><span className="border border-court/30 bg-court/10 px-3 py-2 text-xs font-black text-court">{totalPlayers} listed</span></div><form action="/players" className="mt-5 flex max-w-xl gap-2"><label className="flex-1"><span className="sr-only">Search players</span><input name="q" defaultValue={search} placeholder="Search player or team" className="w-full border border-line p-3"/></label><button type="submit" className="btn-primary">Search</button>{search && <Link href="/players" className="btn-ghost">Clear</Link>}</form><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{players.map((player) => <article key={player.id} className="panel p-4"><div className="flex items-center gap-3"><PlayerAvatar {...player} size="lg"/><div><div className="label">{player.team?.division.isPublic ? `${player.team.division.name}${player.team.group ? ` - ${player.team.group.name}` : ""} - ${player.team.shortName}` : "Confirmed - assignment TBD"}</div><h2 className="text-lg font-black">{formatPlayerDisplayName(player)}</h2><div className="text-xs font-bold text-gray-500">{player.sex} - Confirmed</div></div></div><div className="mt-4 border-t border-line pt-3 text-sm text-gray-600">Playing pairs are matchup-specific and may change between rounds.</div><div className="mt-3 flex flex-wrap gap-1">{player.divisionEntries.filter((entry) => entry.status !== "WITHDRAWN").map((entry) => <span key={entry.id} className="border border-line bg-paper px-2 py-1 text-[10px] font-black uppercase">{entry.division.name}: {entry.status}</span>)}</div></article>)}</div>{!players.length && <div className="mt-6 border border-line bg-white p-8 text-center text-gray-500">No confirmed public players match this search.</div>}{totalPages > 1 && <nav className="mt-6 flex flex-wrap items-center justify-between gap-3 border border-line bg-white p-3 text-sm font-bold"><Link href={pageHref(Math.max(1, currentPage - 1))} className={`btn-ghost px-3 py-2 ${currentPage === 1 ? "pointer-events-none opacity-45" : ""}`}>Previous</Link><span className="text-gray-600">Page {currentPage} of {totalPages}</span><Link href={pageHref(Math.min(totalPages, currentPage + 1))} className={`btn-ghost px-3 py-2 ${currentPage >= totalPages ? "pointer-events-none opacity-45" : ""}`}>Next</Link></nav>}</main>;
}
