...
export default async function GroupPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="public-page mx-auto max-w-7xl px-4 py-5 md:py-8"><TournamentSync initialRevision={revision}/><div className="public-hero"><div><div className="public-kicker">{group.division.name} · group stage</div><h1 className="public-title">{group.name}</h1><p className="public-lede">Standings, qualification status, and the latest matchup results for this group.</p></div><div className="flex flex-wrap gap-2">{divisionGroups.map((item) => <Link key={item.id} href={`/groups/${item.slug}`} className={`btn-ghost rounded-lg ${slug === item.slug ? "border-court bg-court/5 text-court" : ""}`}>{item.name}</Link>)}</div></div><section className="panel mt-6 overflow-hidden"><StandingsTable rows={standings} qualificationByTeam={qualificationByTeam}/></section><section className="mt-8"><div className="public-kicker">{matchups.length} matchups · {gameCounts.length === 1 ? `${gameCounts[0]} matches each` : "variable match counts"}</div><h2 className="mt-1 text-2xl font-black">Matchups & results</h2><div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{matchups.map((matchup) => {
    ...
    return <article key={matchup.id} className={`public-card overflow-hidden p-0 ${matchup.status === "LIVE" ? "border-flame/50 ring-1 ring-flame/20" : ""}`}><div className="flex justify-between gap-3 border-b border-line bg-gray-50 px-4 py-3"><Link href={`/matches/${matchup.id}`} className="label hover:text-court">{matchup.queuePosition !== null ? `Next #${matchup.queuePosition} · ` : ""}{matchup.roundLabel} · Court {matchup.courtLabel || "TBA"}</Link><StatusBadge status={matchup.status} compact/></div><div className="grid grid-cols-[1fr_auto_1fr] items-stretch text-center"><div className={`p-4 ${homeWon ? "bg-emerald-50" : awayWon ? "bg-red-50/60" : ""}`}>{matchup.homeTeam ? <Link href={`/teams/${matchup.homeTeam.id}`} className="group/team block"><strong className={`block text-lg group-hover/team:text-court ${homeWon ? "text-emerald-800" : ""}`}>{matchup.homeTeam.shortName}</strong><span className="text-xs text-gray-500 group-hover/team:text-court">{matchup.homeTeam.name}</span></Link> : <strong className="block text-lg">TBD</strong>}{homeWon && <div className="mt-2 text-[9px] font-black uppercase tracking-widest text-emerald-700">Winner</div>}</div><Link href={`/matches/${matchup.id}`} aria-label={`Open ${matchup.roundLabel}`} className="grid place-items-center bg-ink px-3 text-2xl font-black text-white hover:bg-court">{matchup.homeWins}–{matchup.awayWins}</Link><div className={`p-4 ${awayWon ? "bg-emerald-50" : homeWon ? "bg-red-50/60" : ""}`}>{matchup.awayTeam ? <Link href={`/teams/${matchup.awayTeam.id}`} className="group/team block"><strong className={`block text-lg group-hover/team:text-court ${awayWon ? "text-emerald-800" : ""}`}>{matchup.awayTeam.shortName}</strong><span className="text-xs text-gray-500 group-hover/team:text-court">{matchup.awayTeam.name}</span></Link> : <strong className="block text-lg">TBD</strong>}{awayWon && <div className="mt-2 text-[9px] font-black uppercase tracking-widest text-emerald-700">Winner</div>}</div></div></article>;
  })}</div></section></main>;
}
