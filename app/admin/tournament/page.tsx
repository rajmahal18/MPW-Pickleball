...
import { bracketWinnerQualificationSource, isEarlyQualificationPreview, qualificationSourceOptions } from "@/lib/tournament/bracket-seeding";
...
export default async function TournamentSetup({ searchParams }: { searchParams: Promise<{ success?: string; error?: string; division?: string }> }) {
  ...
  const divisions = await prisma.division.findMany({
    ...
    select: { id: true, name: true, slug: true, sortOrder: true, formatType: true, entrantType: true, sexCategory: true, isPublic: true, _count: { select: { teams: true, matchups: true } } },
    orderBy: [{ sortOrder: "asc" }, { name: "asc" }],
  });
  const selectedId = divisions.some((division) => division.id === query.division) ? String(query.division) : divisions[0]?.id;
  if (!selectedId) return <main className="admin-shell"><AdminNav role={user.role}/><EmptyState text="No divisions are configured yet."/></main>;
  ...
  const selected = await prisma.division.findFirst({
    ...
    include: {
      ...
      teams: { include: {
        group: true,
        players: { select: { id: true, firstName: true, middleInitial: true, lastName: true, displayName: true, participationStatus: true, isActive: true } },
        pairs: { where: { isActive: true }, include: { playerA: { select: { id: true, firstName: true, middleInitial: true, lastName: true, displayName: true } }, playerB: { select: { id: true, firstName: true, middleInitial: true, lastName: true, displayName: true } } }, orderBy: { label: "asc" } },
        _count: { select: { players: true } },
      }, orderBy: [{ groupPosition: "asc" }, { shortName: "asc" }] },
  ...
  if (!selected) return <main className="admin-shell"><AdminNav role={user.role}/><EmptyState text="Selected division was not found."/></main>;
  ...
  const unassignedConfirmed = selected.entrantType === "PAIR"
    ? confirmedEntries.filter((entry) => !pairPlayerIds.has(entry.player.id)).length
    : confirmedEntries.filter((entry) => !entry.player.teamId || !selected.teams.some((team) => team.id === entry.player.teamId)).length;
  ...
  const firstRoundSourceOptions = selected.wildcardMode === "DIRECT" || selected.wildcardMode === "BATTLE"
    ...
    : qualificationSourceOptions(selected.groups, selected.qualifiersPerGroup, selected.wildcardCount);
  const firstRoundConfigurationLocked = firstRoundMatchups.some(matchupLocked);
  const championFinal = selected.matchups.find((matchup) => matchup.bracketTrack === "CHAMPIONSHIP" && matchup.stage === "FINAL" && matchup.winnerTeamId && (matchup.status === "COMPLETED" || matchup.status === "FORFEITED"));
  ...
  return <main className="admin-shell">
    ...
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-gray-600">Configure each event in the same structure you already operate. Team Event keeps lineup rules; fixed-pair Executive events skip that step automatically.</p>
      </div>
      <details className="rounded-lg border border-line bg-white shadow-sm">
        ...
        <form action="/api/admin/tournament-structure" method="post" className="grid w-full gap-3 border-t border-line p-4 sm:w-[420px]">
          ...
          <Select label="Event sex" name="sexCategory" defaultValue=""><option value="">Both / open</option><option value="MALE">Men only</option><option value="FEMALE">Women only</option></Select>
      ...
      <div className="border-b border-line bg-paper px-4 py-3">
        ...
        <div className="mt-2 flex gap-2 overflow-x-auto pb-1">
          {divisions.map((division) => <Link key={division.id} href={`/admin/tournament?division=${division.id}`} className={`shrink-0 rounded-md border px-4 py-3 ${division.id === selected.id ? "border-court bg-court text-white" : "border-line bg-white text-ink hover:border-court"}`}>
            ...
            <div className={`mt-0.5 text-[11px] ${division.id === selected.id ? "text-white/75" : "text-gray-500"}`}>{division._count.teams} {division.entrantType === "PAIR" ? "pairs" : "teams"} · {division._count.matchups} matchups</div>
          </Link>)}
        </div>
      </div>
      <div className="p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2"><StatusBadge>{entrantCopy[selected.entrantType]}</StatusBadge>{selected.sexCategory && <StatusBadge>{selected.sexCategory === "MALE" ? "Men" : "Women"}</StatusBadge>}<StatusBadge tone={selected.isPublic ? "ready" : "warn"}>{selected.isPublic ? "Public" : "Private"}</StatusBadge>{lockedMatchups > 0 && <StatusBadge tone="locked">{lockedMatchups} protected</StatusBadge>}</div>
        </div>
        <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><Stat label={selected.entrantType === "PAIR" ? "Pairs" : "Teams / entries"} value={selected.teams.length}/><Stat label="Groups" value={selected.groups.length}/><Stat label="Future + played matchups" value={selected.matchups.length}/><Stat label="Confirmed players" value={confirmedEntries.length}/></div>
    ...
    <TournamentSetupTabs pairMode={selected.entrantType === "PAIR"}>
      <section id="division-settings" className="scroll-mt-40 rounded-xl border border-line bg-white p-5 shadow-panel">
        ...
        <form key={selected.id} action="/api/admin/tournament-structure" method="post" className="space-y-5">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            ...
            <Select label="Event sex" name="sexCategory" defaultValue={selected.sexCategory ?? ""}><option value="">Both / open</option><option value="MALE">Men only</option><option value="FEMALE">Women only</option></Select>
            {selected.entrantType === "PAIR" ? <>
              <input type="hidden" name="defaultGamesPerMatchup" value="1"/><input type="hidden" name="knockoutGamesPerMatchup" value="1"/>
              <div className="rounded-md border border-court/20 bg-court/5 p-3 text-sm font-bold text-court md:col-span-2">Fixed-pair event · 1 pair match per matchup</div>
            </> : <>
          ...
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="rounded-md border border-line bg-paper px-3 py-2 text-xs font-bold leading-5 text-gray-600"><span className="block text-ink">Scoring</span> Group: 11 sudden death · R16/QF: 11, win by 2, cap 15 · SF/GF/3rd: target 15, hard cap 15</div>
          </div>
          {selected.entrantType === "PAIR" && selected.formatType === "GROUP_KNOCKOUT" && <div className="grid gap-3 rounded-lg border border-violet-300 bg-violet-50 p-4 md:grid-cols-2"><Select label="Wildcard path" name="wildcardMode" defaultValue={selected.wildcardMode} help="Standard uses the qualifier counts below. Direct sends the best remaining seed through. Battle creates a separate tournament for one slot."><option value="STANDARD">Configured qualifiers</option><option value="DIRECT">Direct · best remaining seed</option><option value="BATTLE">Battle tournament</option></Select><Select label="Wildcard battle starts at" name="wildcardBattleSize" defaultValue={String(selected.wildcardBattleSize)} help="Used only for Battle tournament mode."><option value="2">Final · 2 pairs</option><option value="4">Semifinals · 4 pairs</option><option value="8">Quarterfinals · 8 pairs</option></Select></div>}
          {selected.entrantType !== "PAIR" && <><input type="hidden" name="wildcardMode" value="STANDARD"/><input type="hidden" name="wildcardBattleSize" value={selected.wildcardBattleSize}/></>}
          {selected.formatType === "GROUP_KNOCKOUT" ? <div className="grid gap-4 rounded-lg border border-court/20 bg-court/5 p-4 md:grid-cols-2"><Field label={selected.entrantType === "PAIR" ? "Pairs advancing per group" : "Teams advancing per group"} name="qualifiersPerGroup" type="number" min={0} max={16} defaultValue={selected.qualifiersPerGroup} required/><Field label="Wildcard slots" name="wildcardCount" type="number" min={0} max={16} defaultValue={selected.wildcardCount} required/></div> : <><input type="hidden" name="qualifiersPerGroup" value={selected.qualifiersPerGroup}/><input type="hidden" name="wildcardCount" value={selected.wildcardCount}/></>}
      ...
      <section id="teams" className="scroll-mt-40 rounded-xl border border-line bg-white p-5 shadow-panel">
        <SectionHeader eyebrow="Entrants" title={selected.entrantType === "PAIR" ? "Pairs & groups" : "Teams & groups"} action={<div className="flex flex-wrap gap-2">
          ...
          {selected.entrantType === "PAIR" ? <Link href="/admin/players" className="btn-ghost rounded-md px-3 py-2 text-xs">+ Executive pair</Link> : <details className="rounded-md border border-line bg-white"><summary className="cursor-pointer list-none px-3 py-2 text-xs font-black uppercase text-court">+ Team / entry</summary><form action="/api/admin/tournament-structure" method="post" className="grid gap-3 border-t border-line p-3 sm:w-80"><input type="hidden" name="action" value="create-team"/><input type="hidden" name="divisionId" value={selected.id}/><Field label="Team / entrant name" name="name" required/><Field label="Short name" name="shortName" required/><Select label="Group" name="groupId"><option value="">No group</option>{selected.groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</Select><Field label="Group position" name="groupPosition" type="number" min={1} max={99}/><SubmitButton className="btn-primary rounded-md text-xs" pendingLabel="Creating...">Create entry</SubmitButton></form></details>}
        </div>}>{selected.entrantType === "PAIR" ? "Create fixed Executive pairs from the Player Pool, then assign them to groups or future matchups." : "Create teams, assign groups, and edit names."}</SectionHeader>

        {selected.groups.length > 0 && selected.teams.length > 0 && <GroupAssignmentBoard
          divisionId={selected.id}
          groups={selected.groups.map((group) => ({ id: group.id, name: group.name }))}
          teams={selected.teams.map((team) => ({ id: team.id, name: team.name, shortName: team.shortName, groupId: team.groupId, groupPosition: team.groupPosition }))}
          futureGroupMatchupCount={groupMatchups.filter((matchup) => !matchupLocked(matchup)).length}
          assignmentLocked={lockedMatchups > 0}
          entrantLabel={selected.entrantType === "PAIR" ? "pair" : "team"}
        />}

        {selected.groups.length > 0 && <details className="mb-5 rounded-lg border border-line bg-paper p-3">
          <summary className="cursor-pointer text-xs font-black uppercase text-gray-600">Group settings & tiebreaks</summary>
        ...
        {!selected.teams.length && <EmptyState text={selected.entrantType === "PAIR" ? "No Executive pairs yet. Create fixed pairs from the Player Pool." : "No teams or entries yet. Create one above or assign confirmed players from the Player Pool."} action={<Link href="/admin/players" className="btn-primary inline-flex rounded-md">Open Player Pool</Link>} />}
      </section>

      <section id="lineup-rules" className="scroll-mt-40 rounded-xl border border-line bg-white p-5 shadow-panel">
        {selected.entrantType === "PAIR" ? <><SectionHeader eyebrow="Fixed pair event" title="No lineup submission needed">Executive entrants are already locked as pairs. Each scheduled pair-vs-pair matchup becomes one scoreable match automatically.</SectionHeader><div className="rounded-lg border border-court/20 bg-court/5 p-4 text-sm font-bold text-court">Lineup rules are intentionally disabled for this event type.</div></> : <>
      ...
      <section id="schedule" className="scroll-mt-40 rounded-xl border border-line bg-white p-5 shadow-panel">
        <SectionHeader eyebrow="Tournament day" title="Courts & queue">Stack {selected.entrantType === "PAIR" ? "pair" : "team"} matchups by court and call order.</SectionHeader>
      ...
      <section id="matchups" className="scroll-mt-40 rounded-xl border border-line bg-white p-5 shadow-panel">
        ...
        {selected.formatType === "GROUP_KNOCKOUT" && (!selected.autoProgression || !firstKnockoutStage) && <div className="mb-5 rounded-lg border border-line bg-paper p-3 text-xs text-gray-600">First-round seed mapping appears with Auto progression and exactly 4 or 8 Championship qualifiers. Current configured total: <strong>{expectedQualifierCount}</strong>.</div>}
