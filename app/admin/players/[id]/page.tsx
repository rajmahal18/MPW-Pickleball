    ...
    <section className="mt-4 border border-line bg-white">
      ...
      <div className="grid gap-6 p-5 xl:grid-cols-[1fr_300px]">
        <form action="/api/admin/master-data" method="post" className="space-y-5">
          ...
          <section className="border-t border-line pt-5"><div className="label">Assignment</div><div className="mt-3 grid gap-3 sm:grid-cols-[1fr_180px]"><label><span className="label">Team</span><select name="teamId" defaultValue={player.teamId ?? ""} className="mt-1 w-full border border-line p-3 text-sm font-bold"><option value="">Unassigned / player pool</option>{divisions.filter((division) => division.entrantType === "TEAM").map((division) => <optgroup key={division.id} label={division.name}>{division.teams.map((team) => <option key={team.id} value={team.id}>{team.shortName} — {team.name}</option>)}</optgroup>)}</select></label><label className="mt-[17px] flex min-h-12 items-center gap-3 border border-line bg-paper px-3 text-sm font-bold"><input type="checkbox" name="isActive" defaultChecked={player.isActive}/> Active player</label></div><p className="mt-2 text-xs text-gray-500">Recorded play protects historical team assignment. Unplayed future lineup pairings are released automatically if you move the player.</p></section>
        ...
        <aside className="space-y-4">
          ...
          <section className="border border-line bg-white p-4"><div className="label">Division eligibility</div><div className="mt-3 space-y-2">{player.divisionEntries.length ? player.divisionEntries.map((entry) => <div key={entry.id} className="border border-line bg-paper p-2 text-xs"><div className="font-black">{entry.division.name}</div><div className="mt-1 text-gray-500">{entry.status}</div></div>) : <div className="text-sm text-gray-500">No division status set.</div>}</div><form action="/api/admin/master-data" method="post" className="mt-3 grid gap-2"><input type="hidden" name="action" value="set-division-status"/><input type="hidden" name="playerId" value={player.id}/><label><span className="label">Division</span><select name="divisionId" className="mt-1 w-full border border-line p-2 text-xs">{divisions.filter((division) => !division.sexCategory || division.sexCategory === player.sex).map((division) => <option key={division.id} value={division.id}>{division.name}</option>)}</select></label><label><span className="label">Status</span><select name="status" className="mt-1 w-full border border-line p-2 text-xs"><option value="ELIGIBLE">Eligible</option><option value="CONFIRMED">Confirmed</option><option value="UNAVAILABLE">Unavailable</option><option value="WITHDRAWN">Withdrawn</option></select></label><SubmitButton className="btn-ghost" pendingLabel="Saving…">Update eligibility</SubmitButton></form></section>
        </aside>
      </div>
    </section>
  </main>;
