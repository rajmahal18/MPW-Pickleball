      ...
      <div className="divide-y divide-line md:hidden">
        {operationalMatchups.map((matchup) => <article key={matchup.id} className={`p-4 ${matchup.status === "LIVE" ? "bg-flame/5" : matchup.status === "LINEUP_PENDING" ? "bg-amber-50/35" : matchup.status === "READY" ? "bg-emerald-50/25" : ""}`}>
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0"><div className="font-black">{matchup.homeTeam?.shortName || "TBD"} vs {matchup.awayTeam?.shortName || "TBD"}</div><div className="mt-1 text-[11px] font-bold text-gray-500">{matchup.groupLabel || matchup.stage} · {matchup.roundLabel} · Court {matchup.courtLabel || "TBA"}</div></div>
