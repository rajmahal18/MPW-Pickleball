...
export default async function Admin({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  return <main className="admin-shell">
    ...
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <form action="/api/auth/logout" method="post"><SubmitButton className="btn-ghost" pendingLabel="Signing out…">Logout</SubmitButton></form>
    </div>
    <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6"><Stat label="Ongoing matchups" value={liveMatchups} tone={liveMatchups ? "live" : "neutral"}/><Stat label="Live pair games" value={liveGames} tone={liveGames ? "live" : "neutral"}/><Stat label="Pending lineups" value={pendingLineups} tone={pendingLineups ? "warn" : "neutral"}/><Stat label="Ready to play" value={readyMatchups} tone={readyMatchups ? "good" : "neutral"}/><Stat label="Completed pair games" value={completedPairGames} tone="neutral"/><Stat label="Completed matchups" value={completedMatchups} tone="neutral"/></div>
    ...
    <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Quick href="#live-scoring" title="Live Scoring" text="Open active games and encode points without page reloads."/><Quick href="/admin/tournament" title="Tournament Setup" text="Divisions, groups, placement, schedule, and match rules."/><Quick href="/admin/players" title="Player Pool" text="Attendance, team assignment, and division eligibility."/><Quick href="/admin/voting" title="Voting" text="Fan Favorite voting codes and controls."/></div>
    <section id="live-scoring" className="panel mt-6 overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-line p-4"><div><div className="label text-court">Operate</div><h2 className="text-xl font-black uppercase">Live scoring & match controls</h2><p className="mt-1 text-sm text-gray-500">Ordered by urgency: ongoing first, then ready, pending lineup, scheduled, and completed. Game chips also show their own state.</p><div className="mt-2 flex flex-wrap gap-2"><StatusBadge status="LIVE" compact/><StatusBadge status="READY" compact/><StatusBadge status="LINEUP_PENDING" compact/><StatusBadge status="SCHEDULED" compact/><StatusBadge status="COMPLETED" compact/><StatusBadge status="FORFEITED" compact/></div></div><form action="/api/admin/generate-knockout" method="post"><SubmitButton className="btn-ghost" pendingLabel="Recalculating…">Recalculate bracket</SubmitButton></form></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-ink text-left text-white"><tr><th className="p-3">Team matchup</th><th className="p-3">Court</th><th className="p-3">Status</th><th className="p-3">Series</th><th className="p-3">Games</th><th className="p-3">Paper</th></tr></thead><tbody>{operationalMatchups.map((matchup) => <tr className={`border-b border-line ${matchup.status === "LIVE" ? "bg-flame/5" : matchup.status === "LINEUP_PENDING" ? "bg-amber-50/35" : matchup.status === "READY" ? "bg-emerald-50/25" : ""}`} key={matchup.id}><td className="p-3 font-bold">{matchup.homeTeam?.shortName || "TBD"} vs {matchup.awayTeam?.shortName || "TBD"}<div className="text-xs font-normal text-gray-500">{matchup.division.name} · {matchup.groupLabel || matchup.stage} · {matchup.roundLabel}</div></td><td className="p-3 font-bold">{matchup.courtLabel || "TBA"}</td><td className="p-3"><StatusBadge status={matchup.status} compact/></td><td className="p-3"><div className="inline-flex border border-line bg-white px-3 py-2 font-black tabular-nums">{matchup.homeWins}-{matchup.awayWins}</div></td><td className="p-3"><div className="flex min-w-64 flex-wrap gap-2">{matchup.games.map((game) => <Link className={`inline-flex items-center gap-1.5 border px-2 py-1 text-xs font-black ${game.status === "LIVE" ? "border-flame bg-flame/10 text-flame" : game.status === "COMPLETED" || game.status === "FORFEITED" ? "border-court/30 bg-court/10 text-court" : game.status === "INTERRUPTED" ? "border-orange-300 bg-orange-50 text-orange-800" : "border-line bg-white text-gray-600"}`} href={`/admin/score/${game.id}`} key={game.id}><span>G{game.gameNumber}</span><span className="tabular-nums">{game.homeScore}-{game.awayScore}</span><span className="text-[9px] uppercase opacity-70">{game.status === "LIVE" ? "live" : game.status === "COMPLETED" ? "done" : game.status === "FORFEITED" ? "forfeit" : game.status === "INTERRUPTED" ? "paused" : "pending"}</span></Link>)}{!matchup.games.length && <span className="border border-amber-300 bg-amber-50 px-2 py-1 text-xs font-bold text-amber-900">Waiting for both lineups</span>}</div></td><td className="p-3">{scorecardsReady(matchup) ? <Link className="btn-ghost whitespace-nowrap border-emerald-300 bg-emerald-50 px-3 py-2 text-xs text-emerald-800" href={`/admin/matches/${matchup.id}/scorecards`}>✓ Scorecards ready</Link> : <span className="text-xs font-bold text-gray-400">Not ready</span>}</td></tr>)}</tbody></table></div>{!matchups.length && <div className="p-8 text-center text-sm text-gray-500">No assigned matchups are available. Use Tournament Setup for future structure.</div>}</section>
