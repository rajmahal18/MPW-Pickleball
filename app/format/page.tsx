...
export default async function FormatPage() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-8">
    <div className="label text-court">Live tournament configuration</div><h1 className="text-4xl font-black uppercase text-ink">Format Guide</h1>
    <p className="mt-2 max-w-4xl text-sm text-gray-600">This guide is generated from the current tournament configuration. If organizers change a division, game count, team assignment, or future bracket structure, the public guide updates with it.</p>
    <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><StatBox label="Divisions" value={tournament.divisions.length}/><StatBox label="Teams" value={teamCount}/><StatBox label="Confirmed players" value={playerCount}/><StatBox label="Games created" value={gameCount}/></section>
    ...
    <div className="mt-6 space-y-6">{tournament.divisions.map((division) => { const guide = buildDivisionGuide(division); return <section key={division.id} className="overflow-hidden border border-line bg-white">
      <div className="border-l-4 border-gold bg-gradient-to-r from-gold/20 via-white to-court/10 p-4"><div className="label text-court">{guide.formatLabel}</div><div className="flex flex-wrap items-end justify-between gap-3"><h2 className="text-2xl font-black uppercase">{division.name}</h2><span className="text-xs font-black text-gray-500">{guide.confirmedPlayers} confirmed - {guide.assignedPlayers} assigned - {division.teams.length} teams</span></div></div>
      ...
      {division.groups.length > 0 && <div className="border-t border-line p-4"><div className="label">Configured groups</div><div className="mt-2 flex flex-wrap gap-2">{division.groups.map((group) => <Link key={group.id} href={`/groups/${group.slug}`} className="btn-ghost px-3 py-2 text-xs">{group.name} - {group.teams.length} teams</Link>)}</div></div>}
    </section>; })}</div>
    <div className="mt-6 flex flex-wrap gap-3"><Link href="/bracket" className="btn-primary">View knockout board</Link><Link href="/games" className="btn-ghost">View games</Link><Link href="/players" className="btn-ghost">View player pool</Link></div>
  </main>;
}
