...
- Team identity is logo-driven: Superadmins upload one official JPEG/PNG/WebP logo in Tournament Setup, the server derives and persists a contrast-safe nullable palette once, and shared team identity components fall back to the MPW theme for legacy, missing, monochrome, or failed extractions. Existing managed logos can be processed with `npm run branding:backfill`; image analysis never runs during page rendering.
