...
- Finalizing, reopening, forfeiting, or correcting an individual pair match recalculates the parent team matchup immediately. Group standings surface live NPD and total points scored from decided pair matches while a team matchup is still live; matchup P/W/L remain terminal-only.
...
- Group standings now use the compact visible columns `P`, `W`, `L`, `NPD`, and `TP`. `NPD` is total points scored minus total points conceded across decided pair matches; `TP` is accumulated points scored. P/W/L remain terminal-only. After the team-matchup record, the official tiebreak sequence is Total Pair Match Wins → NPD → head-to-head → Total Points Scored.
...
- Group ranking now uses the organizer-approved tiebreak order after team-matchup W/L: **A. Total Pair Match Wins, B. Net Point Differential, C. Head-to-head among teams still tied after A/B, D. Total Points Scored**. Exact ties still require organizer resolution only after the group stage is terminal.
- Public group standings display only **P / W / L / NPD / TP**. Pair-match wins remain tracked as the first tiebreak metric without adding another visible standings column.
...
- Wide operational tables were replaced or supplemented with purpose-focused mobile cards on Control, Player Pool, Audit, Checkpoints, and Voting. Tournament Setup hides the bulk schedule grid on phones while retaining individual matchup controls.
