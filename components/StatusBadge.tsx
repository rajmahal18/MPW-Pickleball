...
const STATUS_STYLES: Record<string, string> = {
  ...
  FORFEITED: "border-amber-400 bg-amber-50 text-amber-900",
...
export default function StatusBadge({ status, label, compact = false, pulse = false }: StatusBadgeProps) {
  ...
  return <span className={`inline-flex items-center gap-1.5 rounded-full border font-black uppercase tracking-wider ${compact ? "px-2 py-1 text-[10px]" : "px-2.5 py-1.5 text-[11px]"} ${style}`}>
