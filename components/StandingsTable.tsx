      ...
      <tbody>{rows.map((row) => {
        ...
        return <tr key={row.team.id} className={`border-b border-line align-top transition-colors last:border-0 ${rowTone(outcome)}`}>
          ...
          <td className="min-w-0 px-1.5 py-3">
            <div className={`${outcome === "QUALIFIED" ? "text-emerald-950" : outcome === "ELIMINATED" ? "text-red-950" : ""}`} title={row.team.name}><TeamIdentity team={row.team} variant="micro" fullName/></div>
