...
export default function FanFavoriteExperience({ players, initialCode = "", initialSnapshot }: { players: Player[]; initialCode?: string; initialSnapshot?: FanFavoriteSnapshot }) {
  ...
  return <div className="space-y-6">
    ...
    <nav className="grid grid-cols-2 overflow-hidden rounded-xl border border-line bg-white p-1 shadow-sm" aria-label="Fan Favorite sections">
      <button type="button" onClick={() => setActiveTab("VOTE")} className={`flex min-h-11 items-center justify-center gap-2 rounded-lg px-4 text-sm font-black ${activeTab === "VOTE" ? "bg-court text-white" : "text-gray-600 hover:bg-paper"}`}><Vote className="h-4 w-4"/> Vote</button>
      <button type="button" onClick={() => setActiveTab("CODES")} className={`flex min-h-11 items-center justify-center gap-2 rounded-lg px-4 text-sm font-black ${activeTab === "CODES" ? "bg-ink text-white" : "text-gray-600 hover:bg-paper"}`}><Ticket className="h-4 w-4"/> Codes</button>
    </nav>
    ...
    <div className="grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
      {activeTab === "VOTE" ? <section data-motion-reveal className="overflow-hidden rounded-2xl border border-line bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line bg-white px-5 py-4"><div><div className="public-kicker">Ballot</div><h2 className="text-2xl font-black tracking-tight">Cast your votes</h2></div><Vote className="h-5 w-5 text-flame"/></div>
...
function PublicCodeDrops({ onUseCode }: { onUseCode: (code: string) => void }) {
  const [snapshot, setSnapshot] = useState<PublicVotingCodeSnapshot>({ latestBatch: null, availableCodes: [], availableCount: 0, nextBatch: null, serverTime: new Date().toISOString() });
  ...
  useEffect(() => {
    ...
    const schedule = () => {
      if (stopped) return;
      if (timer) window.clearTimeout(timer);
      const jitter = FAN_FAVORITE_CODE_POLL_INTERVAL_MS * PUBLIC_POLL_JITTER_RATIO * (Math.random() * 2 - 1);
      timer = window.setTimeout(run, Math.max(1500, Math.round(FAN_FAVORITE_CODE_POLL_INTERVAL_MS + jitter)));
    };
    const run = async () => {
      if (stopped) return;
      ...
      try {
        ...
        if (response.ok) {
          const next: PublicVotingCodeSnapshot = await response.json();
          setSnapshot(next);
          setClockOffset(new Date(next.serverTime).getTime() - Date.now());
        }
      } catch (error) { if (!(error instanceof DOMException && error.name === "AbortError")) { /* retry */ } }
  ...
  const nextDropMs = snapshot.nextBatch ? new Date(snapshot.nextBatch.releaseAt).getTime() - (tick + clockOffset) : null;
...
function CrowdLeaderPoster({ ranking, label, tone }: { ranking?: Ranking; label: string; tone: "male" | "female" }) {
  ...
  return <article className="group relative overflow-hidden rounded-2xl border border-white/15 bg-white/10 p-4 backdrop-blur transition hover:-translate-y-0.5 hover:bg-white/15"><div className={`absolute inset-y-0 ${tone === "male" ? "left-0 bg-court/25" : "right-0 bg-gold/20"} w-1/2 blur-3xl`}/><div className="relative flex items-center gap-4"><Link href={`/players/${player.id}`} className="relative shrink-0"><PlayerAvatar {...player} size="lg"/><span className="absolute -right-1 -top-1 grid h-7 w-7 place-items-center rounded-full bg-gold text-ink shadow"><Crown className="h-3.5 w-3.5" fill="currentColor"/></span></Link><div className="min-w-0 flex-1"><div className="text-[9px] font-black uppercase tracking-[.18em] text-gold">{label}</div><div className="mt-1 flex min-w-0 items-center gap-1.5"><Link href={`/players/${player.id}`} className="truncate text-xl font-black tracking-tight hover:text-gold">{formatPlayerDisplayName(player)}</Link><GenderIndicator sex={player.sex} className="text-xl"/></div><div className="mt-1 truncate text-xs font-bold text-white/60">{player.team ? <Link href={`/teams/${player.team.id}`} className="hover:text-gold hover:underline">{player.team.shortName}</Link> : "Player pool"}</div><div className="mt-3 flex items-end gap-2"><strong className="text-2xl font-black">{ranking.votes}</strong><span className="pb-1 text-[9px] font-black uppercase tracking-widest text-white/45">votes · {ranking.percentage}%</span></div></div></div></article>;
}

function PlayerPicker({ title, tone, players, search, setSearch, selectedPlayerId, setSelectedPlayerId }: { title: string; tone: "male" | "female"; players: Player[]; search: string; setSearch: (value: string) => void; selectedPlayerId: string; setSelectedPlayerId: (value: string) => void; }) {
  const accent = tone === "male" ? "border-court/20 bg-court/5" : "border-pink-200 bg-pink-50/40";
  return <div className={`overflow-hidden rounded-xl border ${accent}`}><div className="border-b border-white/80 p-3.5"><div className="flex items-center justify-between gap-2"><h3 className="flex items-center gap-1.5 font-black">{title}<GenderIndicator sex={tone === "male" ? "MALE" : "FEMALE"} className="text-lg"/></h3></div><input value={search} onChange={(event) => setSearch(event.target.value)} className="filter-control mt-3 bg-white" placeholder="Search player or team"/></div><div className="max-h-[390px] space-y-2 overflow-y-auto p-2.5">{players.map((player) => { const selected = selectedPlayerId === player.id; return <label key={player.id} className={`grid cursor-pointer grid-cols-[auto_auto_minmax(0,1fr)] items-center gap-3 rounded-xl border bg-white p-3 transition ${selected ? "border-flame bg-gold/10 ring-2 ring-gold/40" : "border-line hover:border-court/40 hover:shadow-sm"}`}><input type="radio" name={tone} value={player.id} checked={selected} onChange={() => setSelectedPlayerId(player.id)} className="h-4 w-4 accent-current"/><PlayerAvatar {...player} size="md"/><span className="min-w-0"><span className="flex min-w-0 items-center gap-1"><strong className="truncate text-sm leading-tight sm:text-base">{formatPlayerDisplayName(player)}</strong><GenderIndicator sex={player.sex} className="text-base"/></span><span className="mt-1 block truncate text-xs font-semibold text-gray-500">{player.team?.shortName ?? "Unassigned"}</span></span></label>; })}{!players.length && <div className="p-6 text-center text-sm text-gray-500">No eligible players found.</div>}</div></div>;
}
