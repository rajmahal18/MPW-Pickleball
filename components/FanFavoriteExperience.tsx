...
function CrowdLeaderPoster({ ranking, label, tone }: { ranking?: Ranking; label: string; tone: "male" | "female" }) {
  ...
  return <article className="group relative overflow-hidden rounded-2xl border border-white/15 bg-white/10 p-4 backdrop-blur transition hover:-translate-y-0.5 hover:bg-white/15"><div className={`absolute inset-y-0 ${tone === "male" ? "left-0 bg-court/25" : "right-0 bg-gold/20"} w-1/2 blur-3xl`}/><div className="relative flex items-center gap-4"><Link href={`/players/${player.id}`} className="relative shrink-0"><PlayerAvatar {...player} size="lg"/><span className="absolute -right-1 -top-1 grid h-7 w-7 place-items-center rounded-full bg-gold text-ink shadow"><Crown className="h-3.5 w-3.5" fill="currentColor"/></span></Link><div className="min-w-0 flex-1"><div className="text-[9px] font-black uppercase tracking-[.18em] text-gold">{label}</div><div className="mt-1 flex min-w-0 items-center gap-1.5"><Link href={`/players/${player.id}`} className="break-words text-xl font-black leading-tight tracking-tight hover:text-gold">{formatPlayerDisplayName(player)}</Link><GenderIndicator sex={player.sex} className="text-xl"/></div><div className="mt-1 text-xs font-bold text-white/60">{player.team ? <Link href={`/teams/${player.team.id}`} className="hover:text-gold hover:underline">{player.team.shortName}</Link> : "Player pool"}</div><div className="mt-3 flex items-end gap-2"><strong className="text-2xl font-black">{ranking.votes}</strong><span className="pb-1 text-[9px] font-black uppercase tracking-widest text-white/45">votes · {ranking.percentage}%</span></div></div></div></article>;
}
