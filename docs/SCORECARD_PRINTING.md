...
4. Review the auto-filled team names, players, round, and court. Group identity comes from the configured round/matchup (for example, **Group A Match 3**) and is not entered separately on the scorecard.
...
- A4 landscape
- two **portrait** scorecards side-by-side per physical sheet
- each scorecard uses roughly half of the printable A4 width and the full printable height
...
- the umpire signature band is intentionally compact; available vertical space is prioritized for player names, score writing, and team-representative signatures
